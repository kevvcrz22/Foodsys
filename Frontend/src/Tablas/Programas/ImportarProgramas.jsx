...
/* ── Paleta del modulo de Programas (indigo) ── */
const CL = {
  header:   "bg-gradient-to-r from-indigo-600 to-indigo-500",
  badge:    "bg-indigo-100 text-indigo-700",
  btn:      "bg-indigo-600 hover:bg-indigo-700 text-white",
  btnLight: "bg-indigo-50 hover:bg-indigo-100 text-indigo-700",
...
/* ─────────────────────────────────────────────
   ImportarProgramas — componente principal
   Props:
     onClose  : cierra el modal desde el padre
     reload   : refresca la lista de programas
───────────────────────────────────────────── */
const ImportarProgramas = ({ onClose, reload }) => {
  ...
  */
  const DescargarPlantilla = async () => {
    try {
      setCargando(true);
      const Resp = await apiAxios.get("/api/Programa/plantilla-excel", { responseType: "blob" });
  ...
  // ── Paso 2a: captura el archivo seleccionado por el usuario ──
  ...
  // ── Paso 2b: envia el archivo al backend y recibe el preview ──
  /*
    Se usa FormData porque el backend espera un archivo multipart.
    El campo debe llamarse "file" (nombre configurado en multer).
    El backend devuelve { total, data: [{ Nom_Programa, Are_Programa }] }
  ...
  */
  const ObtenerPreview = async () => {
    ...
    try {
      ...
      const Resp = await apiAxios.post("/api/Programa/preview-import", Form, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      const Datos = Resp.data.data || [];
      setPreview(Datos);
      // Por defecto todos los registros vienen seleccionados
  ...
  // ── Paso 3: alterna la seleccion de una fila individual ──
  const ToggleFila = (Idx) =>
    setSeleccion((Prev) =>
      Prev.includes(Idx) ? Prev.filter((I) => I !== Idx) : [...Prev, Idx]
    );

  // Selecciona o deselecciona todas las filas del preview
  const ToggleTodos = () =>
    setSeleccion(Seleccion.length === Preview.length ? [] : Preview.map((_, I) => I));

  // ── Paso 3b: envia solo las filas aprobadas al backend ──
  /*
    El backend recibe el array de objetos y por cada uno:
      1. Valida campos obligatorios.
      2. Busca duplicado por Nom_Programa.
      3. Persiste si pasa las validaciones.
  ...
  */
  const ConfirmarImportacion = async () => {
    ...
    try {
      setCargando(true);
      setError("");
      const ProgramasSeleccionados = Seleccion.map((Idx) => Preview[Idx]);
      const Resp = await apiAxios.post("/api/Programa/importar-seleccionados", {
      ...
      <div className="relative bg-white w-full sm:max-w-xl rounded-t-3xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[95vh] overflow-hidden z-10">
        ...
        <div className={`${CL.header} px-5 py-4 flex items-center justify-between shrink-0`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-indigo-100 font-semibold m-0 uppercase tracking-wide">Importacion masiva</p>
              <p className="text-white font-bold text-sm m-0">Programas desde Excel</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-4">
                ...
                <p className="text-[12px] text-slate-500 mb-3 leading-relaxed">
                  La plantilla contiene las columnas exactas que el sistema espera:
                  <strong> Nombre del Programa</strong> y <strong>Nombre del Area</strong>.
              ...
              <div>
                ...
                {/* Zona drag-and-drop visual */}
                <label
                  htmlFor="InputPrograma"
                  className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-indigo-200 rounded-2xl py-8 px-4 cursor-pointer bg-indigo-50/50 hover:bg-indigo-50 transition-colors">
          ...
          {/* ══ PASO 2: Tabla de preview con checkboxes ══ */}
          {Paso === 2 && (
            <div className="flex flex-col gap-4">
              {/* Resumen y selector global */}
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div>
                  <p className="font-semibold text-slate-800 text-sm m-0">
                    Revisa y selecciona los programas
                  </p>
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    {Seleccion.length} de {Preview.length} seleccionados
                  </p>
                </div>
                <button onClick={ToggleTodos}
                  ...
                  {Seleccion.length === Preview.length
              ...
              <div className="flex flex-col gap-2 max-h-64 overflow-y-auto pr-1">
                {Preview.map((Fila, Idx) => {
                  ...
                  return (
                    <label key={Idx}
                      className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${Marcado ? "bg-indigo-50 border-indigo-200" : "bg-slate-50 border-slate-100 opacity-60"}`}>
                      <input type="checkbox" className={`mt-0.5 w-4 h-4 ${CL.check} shrink-0`}
                        checked={Marcado} onChange={() => ToggleFila(Idx)} />
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-slate-800 text-sm m-0 truncate">
                          {Fila.Nom_Programa || <span className="text-red-400 italic">Sin nombre</span>}
                        </p>
                        <p className="text-[11px] text-slate-400 m-0">
                          Area: {Fila.Are_Programa || "—"}
                        </p>
                      </div>
                    </label>
                  );
              ...
              <div className="flex gap-3">
                ...
                <button onClick={ConfirmarImportacion} disabled={!Seleccion.length || Cargando}
                  ...
                  Importar {Seleccion.length} programa{Seleccion.length !== 1 ? "s" : ""}
                </button>
              </div>
            </div>
          )}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 text-center">
                  ...
                  <p className="text-[11px] font-semibold text-emerald-600 uppercase tracking-wide mt-1 m-0">Creados</p>
                </div>
                <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 text-center">
                  <p className="text-3xl font-extrabold text-amber-700 m-0">{Resultado.omitidos}</p>
                  <p className="text-[11px] font-semibold text-amber-600 uppercase tracking-wide mt-1 m-0">Omitidos</p>
                </div>
              </div>
              ...
              {Resultado.errores?.length > 0 && (
                <div className="bg-red-50 border border-red-100 rounded-2xl p-4">
                  <p className="font-semibold text-red-700 text-[12px] uppercase mb-2">
...
export default ImportarProgramas;