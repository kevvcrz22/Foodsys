...
const CrudRoles = () => {
  ...
  const columnsTable = [
    { name: "ID", selector: (row) => row.Id_Rol, sortable: true },
    { name: "Nombre Rol", selector: (row) => row.Nom_Rol, sortable: true },
    {
      name: "Acciones",
      cell: (row) => (
        <button
  ...
  const newListRol = Roles.filter((p) =>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative z-10 max-h-[95vh] overflow-hidden flex flex-col animate-fadeIn">
              ...
              <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4 flex-shrink-0">
                <h2 className="text-xl font-semibold text-gray-800 flex items-center gap-2">
                  <i className={`bi ${RolSeleccionado ? 'bi-pencil-square' : 'bi-plus-circle'}`}></i>
                  {RolSeleccionado ? "Editar Rol" : "Registrar Rol"}
                </h2>
                <button
                  onClick={hideModal}
              ...
              <div className="px-6 py-4 overflow-y-auto flex-1">
                <RolesForm 
                  hideModal={hideModal}
                  programa={RolSeleccionado}
                  actualizarLista={getAllRoles}
                />
              </div>
            </div>
          </div>
