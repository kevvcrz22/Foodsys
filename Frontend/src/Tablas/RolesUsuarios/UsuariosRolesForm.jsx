...
const UsuariosRolForm = ({ hideModal, data, Edit, reload }) => {
    ...
    useEffect(() => {
        if (data) {
            setId_UsuarioRol(data.Id_UsuariosRol); // ✅ corregido
            setId_Usuario(data.Id_Usuario);
            setId_Rol(data.Id_Rol);
            setFec_Ingreso(data.Fec_Ingreso?.split("T")[0]);
    ...
    const gestionarForm = async (e) => {
        ...
        try {
            if (!Edit) {
                await apiAxios.post("/api/UsuariosRoles", { // ✅ corregido
                    Id_Usuario,
                    Id_Rol,
                    Fec_Ingreso
                });
                ...
                await apiAxios.put(`/api/UsuariosRoles/${Id_UsuarioRol}`, { // ✅ corregido
                    Id_Usuario,
                    Id_Rol,
                    Fec_Ingreso
                });
    ...
    return (
        <form onSubmit={gestionarForm} className="space-y-4">
            ...
            <div>
                <label className="block text-sm font-medium mb-2">Id Usuario</label>
                <input
                    type="number"
            ...
            <div>
                <label className="block text-sm font-medium mb-2">Id Rol</label>
                <input
                    type="number"
                    className="w-full px-4 py-2 border rounded-lg"
                    value={Id_Rol}
